...
let get_fun ~loc type_name pld_name pld_type params =
  value_description ~loc
    ~name:(Loc.make ~loc ("get_" ^ Loc.txt pld_name))
    ~type_:
      (ptyp_arrow ~loc Nolabel
         (ptyp_constr ~loc (Loc.make ~loc (Lident type_name)) params)
         pld_type)
    ~prim:[]

let set_fun ~loc type_name pld_name pld_type params =
  value_description ~loc
    ~name:(Loc.make ~loc ("set_" ^ Loc.txt pld_name))
    ~type_:
      (ptyp_arrow ~loc Nolabel
         (ptyp_constr ~loc (Loc.make ~loc (Lident type_name)) params)
         (ptyp_arrow ~loc Nolabel pld_type
            (ptyp_constr ~loc (Loc.make ~loc (Lident type_name)) params)))
    ~prim:[]

let to_label_or_opt pld_name pld_type exp =
  match pld_type.ptyp_desc with
  | Ptyp_constr ({txt= Lident "option"; _}, n) ->
    Optional (Loc.txt pld_name), List.hd n, exp
  | _ -> Labelled (Loc.txt pld_name), pld_type, exp

let create_fun ~loc type_name labels params =
  value_description ~loc ~name:(Loc.make ~loc "make")
    ~type_:
      (List.fold_right
         (fun {pld_name; pld_type; _} exp ->
           let label, type_, exp = to_label_or_opt pld_name pld_type exp in
           ptyp_arrow ~loc label type_ exp)
         labels
         (ptyp_arrow ~loc Nolabel
            (ptyp_constr ~loc (Loc.make ~loc (Lident "unit")) [])
            (ptyp_constr ~loc (Loc.make ~loc (Lident type_name)) params)))
    ~prim:[]

let create_abstract_type ~loc type_name params =
  type_declaration ~loc ~name:type_name ~params ~cstrs:[] ~kind:Ptype_abstract
...
let from_td
    ~loc {ptype_name; ptype_kind; ptype_loc; ptype_params; ptype_manifest; _} =
...
let extension =
  Extension.V3.declare_inline "accessor" Extension.Context.signature_item
    Ast_pattern.(psig (psig_type recursive (__ ^:: nil) ^:: nil))
    expand

let rule = Context_free.Rule.extension extension
let () = Driver.register_transformation ~rules:[rule] "accessor"
