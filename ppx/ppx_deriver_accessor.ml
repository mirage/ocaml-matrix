    ...
    ~expr:
      (List.fold_right
        (fun {pld_name; pld_type; _} exp ->
          (pexp_fun ~loc (to_label_or_opt pld_name pld_type) None
            (ppat_var ~loc
              (Loc.make ~loc (Loc.txt pld_name)))
            exp))
        labels
        (pexp_fun ~loc Nolabel None
          (ppat_construct ~loc
...
let from_label ~loc ~single type_name {pld_name; _} =
  if single
  then
    [get_fun ~loc type_name pld_name; set_fun_single ~loc pld_name]
  else
    [get_fun ~loc type_name pld_name; set_fun ~loc type_name pld_name]
...
let from_td ~loc {ptype_name; ptype_kind; ptype_loc; ptype_manifest; _} =
  ...
  let values =
    ...
    | Ptype_record labels ->
      (create_fun ~loc labels)::(List.flatten (List.map (from_label ~loc ~single:(List.length labels = 1) type_name) labels))
...
let str_type_decl_generator =
  Deriving.Generator.make_noarg expand
