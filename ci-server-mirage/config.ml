...
let client_port =
  let doc = Key.Arg.info ~doc:"Listening port for the client API." [ "client_port" ] in
  Key.(create "client_port" Arg.(opt int 8008 doc))

let federation_port =
  let doc = Key.Arg.info ~doc:"Listening port for the federation API." [ "federation_port" ] in
...
let remote =
  ...
  Key.(create "remote" Arg.(opt string "git://127.0.0.1/ocaml-matrix-store" doc))
...
let dream =
  foreign "Unikernel.Make"
    ...
    ~keys:Key.([ abstract hostname
               ; abstract client_port
               ; abstract federation_port
               ; abstract priv_key
               ; abstract remote ])
    (console @-> random @-> time @-> mclock @-> pclock @-> stackv4v6 @-> dns @-> paf @-> mimic @-> job)
...
let mimic_impl =
  mimic_impl default_random stackv4v6
    default_monotonic_clock default_posix_clock default_time

let () = register "dream"
  [ dream $ console $ random $ time $ mclock $ pclock $ stackv4v6 $ dns $ paf time stackv4v6 $ mimic_impl ]
