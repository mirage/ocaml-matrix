...
module Make
    ...
    (Dns : DNS)
    (* XXX(dinosaure): ask @hannesm to provide a signature. *)
...
struct
  ...
  let with_tls port ctx =
    ...
    Mimic.(
      fold Paf.tls_edn
        Fun.
          [
            req Client.scheme; opt Client.domain_name; dft tls default_tls_cfg;
            req stack; req Client.ipaddr; dft Client.port port;
          ]
        ~k ctx)
  ...
  let http_ctx stack port dns =
    ...
    |> with_sleep
    |> with_tls port
    (* domain_name -> tls -> stack -> ipaddr -> port => (domain_name * tls * stack * ipaddr * port) *)
  ...
  let start _console _ _ _ _ stackv4v6 dns _ ctx =
    let ctx =
      Mimic.merge ctx (http_ctx stackv4v6 (Key_gen.federation_port ()) dns)
    in
