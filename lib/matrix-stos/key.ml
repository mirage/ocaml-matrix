...
struct
  ...
  let encoding =
    let to_tuple t =
      t.server_name, t.verify_keys, t.old_verify_keys, t.signatures, t.valid_until_ts
    in
    let of_tuple v =
      let server_name, verify_keys, old_verify_keys, signatures, valid_until_ts = v in
...
module Indirect_query =
struct
  module Query =
  struct
    type t =
      { minimum_valid_until_ts: int option
      } [@@deriving accessor]
  ...
  let path server_name key_id = "/_matrix/key/v2/query/" ^ server_name ^ "/" ^ key_id
    (* The use of key id is deprecated, servers should ask for all the keys
    * instead
    *)
  ...
  struct
    ...
    let encoding =
      ...
      let with_tuple =
        obj1
          (req "server_keys" (list Server_key.encoding))
      in
      conv to_tuple of_tuple with_tuple
  end
  ...
  struct
    ...
    struct
      ...
      let encoding =
        ...
        let with_tuple =
          obj1
            (opt "minimum_valid_until_ts" int)
        in
        conv to_tuple of_tuple with_tuple
    end
    ...
    let encoding =
      ...
      let with_tuple =
        obj1
          (req "server_keys" (assoc (assoc Query_criteria.encoding)))
      in
      conv to_tuple of_tuple with_tuple
  end
