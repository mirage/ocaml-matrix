...
module Request = struct
  ...
  let encoding =
    let to_tuple t = t.new_version in
    let of_tuple v =
      let new_version = v in
      {new_version} in
    let with_tuple = obj1 (req "new_version" string) in
    conv to_tuple of_tuple with_tuple
end

module Response = struct
  type t = {replacement_room: string} [@@deriving accessor]

  let encoding =
    let to_tuple t = t.replacement_room in
    let of_tuple v =
