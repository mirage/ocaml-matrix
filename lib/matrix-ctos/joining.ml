...
module Join_with_id = struct
  ...
  module Request = struct
    ...
    let encoding =
      let to_tuple t = t.third_party_signed in
      let of_tuple v =
        let third_party_signed = v in
        {third_party_signed} in
      let with_tuple = obj1 (opt "third_party_signed" unit) in
      conv to_tuple of_tuple with_tuple
  end
...
module Join = struct
  module Query = struct
    type t = {server_name: string list option} [@@deriving accessor]
