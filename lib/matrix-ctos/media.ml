...
struct
  ...
  let path server_name media_id = "_matrix/media/r0/download/" ^ server_name ^ "/" ^ media_id
...
struct
  ...
  let path server_name media_id filename = "_matrix/media/r0/download/" ^ server_name ^ "/" ^ media_id ^ "/" ^ filename
end
...
struct
  ...
  struct
    ...
    let args t =
      let l = ["width", [Int.to_string t.width]; "height", [Int.to_string t.height]] in
      let l =
        match t.rezising_method with
          | None -> l
          | Some rezising_method -> ("method", [Rezising.to_string rezising_method])::l
      in
      match t.allow_remote with
        | None -> l
        | Some allow_remote -> ("allow_remote", [Bool.to_string allow_remote])::l

    (* handle the header from the response *)
  end

  let path server_name media_id = "_matrix/media/r0/thumbnail/" ^ server_name ^ "/" ^ media_id
