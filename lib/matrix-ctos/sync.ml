...
struct
  ...
  let args t =
    ...
    let pp ppf t =
      Fmt.(pf ppf "{ filter: %a; since: %a; full_state: %a; set_presence: %a; timeout: %a }"
        Dump.(option string) t.filter
        Dump.(option string) t.since
        Dump.(option bool) t.full_state
        Dump.(option Presence.pp) t.set_presence
        Dump.(option int) t.timeout)
end
...
struct
  ...
  let encoding =
    let to_tuple t =
      t.next_batch, t.rooms, t.presence, t.account_data, t.to_device, t.device_lists, t.device_one_time_keys_count, t.groups
    in
    let of_tuple v =
      let next_batch, rooms, presence, account_data, to_device, device_lists, device_one_time_keys_count, groups = v in
      { next_batch; rooms; presence; account_data; to_device; device_lists; device_one_time_keys_count; groups }
    in
    let with_tuple =
      obj8
        (req "next_batch" string)
