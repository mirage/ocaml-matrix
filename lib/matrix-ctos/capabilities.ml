...
module Capability = struct
  ...
  module Room_versions = struct
    module Stability = struct
      type t = Stable | Unstable

      let encoding = string_enum ["stable", Stable; "unstable", Unstable]
    end

    type t = {default: string; available: (string * Stability.t) list}
    [@@deriving accessor]

    let encoding =
      let to_tuple t = t.default, t.available in
      let of_tuple v =
        let default, available = v in
        {default; available} in
      let with_tuple =
        obj2 (req "default" string) (req "available" (assoc Stability.encoding))
      in
      conv to_tuple of_tuple with_tuple
  end
    ...
    let of_tuple v =
      let f = function
        ...
        | s, ez ->
          ...
          Capability.Custom {t with name= s} in
