...
module Protocol = struct
  module Field_type = struct
    type t = {regexp: string; placeholder: string} [@@deriving accessor]

    let encoding =
      let to_tuple t = t.regexp, t.placeholder in
      let of_tuple v =
        let regexp, placeholder = v in
        {regexp; placeholder} in
      let with_tuple = obj2 (req "regexp" string) (req "placeholder" string) in
  ...
  module Instance = struct
    ...
    let encoding =
      let to_tuple t = t.desc, t.icon, t.fields, t.network_id in
      let of_tuple v =
        let desc, icon, fields, network_id = v in
        {desc; icon; fields; network_id} in
      let with_tuple =
        obj4 (req "desc" string) (opt "icon" string)
          (req "fields" (assoc string))
          (req "network_id" string) in
      conv to_tuple of_tuple with_tuple
  end
  ...
  let encoding =
    let to_tuple t =
      t.user_fields, t.location_fields, t.icon, t.field_types, t.instances in
    let of_tuple v =
      let user_fields, location_fields, icon, field_types, instances = v in
      {user_fields; location_fields; icon; field_types; instances} in
    let with_tuple =
      obj5
        ...
        (req "instances" (list Instance.encoding)) in
    conv to_tuple of_tuple with_tuple
end

module Location = struct
  type t = {alias: string; protocol: string; fields: (string * string) list}
  [@@deriving accessor]

  let encoding =
    let to_tuple t = t.alias, t.protocol, t.fields in
    let of_tuple v =
      let alias, protocol, fields = v in
      {alias; protocol; fields} in
    let with_tuple =
      obj3 (req "alias" string) (req "protocol" string)
        (req "fields" (assoc string)) in
    conv to_tuple of_tuple with_tuple
end

module User = struct
  type t = {user_id: string; protocol: string; fields: (string * string) list}
  [@@deriving accessor]

  let encoding =
    let to_tuple t = t.user_id, t.protocol, t.fields in
    let of_tuple v =
      let user_id, protocol, fields = v in
      {user_id; protocol; fields} in
    let with_tuple =
      obj3 (req "userid" string) (req "protocol" string)
        (req "fields" (assoc string)) in
    conv to_tuple of_tuple with_tuple
end
...
module Get_location = struct
  module Query = struct
    type t = {search_fields: string option} [@@deriving accessor]
...
module Get_user = struct
  module Query = struct
    type t = {fields: (string * string list) list option} [@@deriving accessor]
...
module Location_from_alias = struct
  module Query = struct
    type t = {alias: string} [@@deriving accessor]
