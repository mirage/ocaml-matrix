...
struct
  ...
  let encoding =
    let to_tuple t =
      t.heroes, t.joined_member_count, t.invited_member_count
    in
    let of_tuple v =
    ...
    let with_tuple =
      obj3
        ...
        (opt "m.invited_member_count" int)
    in
...
struct
  ...
  let encoding =
    let to_tuple t =
      t.events, t.limited, t.prev_batch
    in
    let of_tuple v =
      let events, limited, prev_batch = v in
      {events; limited; prev_batch}
    in
    let with_tuple =
      obj3
...
struct
  ...
  struct
    ...
    let encoding =
      let to_tuple t =
        t.highlight_count, t.notification_count
      in
      let of_tuple v =
      ...
      let with_tuple =
        obj2
          (opt "highlight_count" int)
          (opt "notification_count" int)
      in
      conv to_tuple of_tuple with_tuple
  end
  ...
  let encoding =
    let to_tuple t =
      t.summary, t.state, t.timeline, t.ephemeral, t.account_data, t.unread_notifications
    in
    let of_tuple v =
      let summary, state, timeline, ephemeral, account_data, unread_notifications = v in
...
struct
  ...
  let encoding =
    let to_tuple t =
      t.state, t.timeline, t.account_data
    in
    let of_tuple v =
      let state, timeline, account_data = v in
      { state; timeline; account_data }
    in
    let with_tuple =
      obj3
        (opt "state"
...
let encoding =
  ...
  let with_tuple =
    obj3
      (req "join"
        (assoc Joined_room.encoding))
      (req "invite"
        (assoc Invited_room.encoding))
      (req "leave"
        (assoc Left_room.encoding))
  in
  conv to_tuple of_tuple with_tuple
