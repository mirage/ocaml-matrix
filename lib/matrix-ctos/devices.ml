...
struct
  ...
  let encoding =
    let to_tuple t =
      t.user_id, t.device_id, Some t.display_name, Some t.last_seen_ip, Some t.last_seen_ts
    in
    let of_tuple v =
      ...
      in
      let last_seen_ip =
        match last_seen_ip with
        | Some d -> d
        | None -> None
      in
      let last_seen_ts =
        match last_seen_ts with
        | Some d -> d
        | None -> None
      in
      { user_id; device_id; display_name; last_seen_ip; last_seen_ts }
    in
    let with_tuple =
      obj5
        (opt "user_id" string)
...
struct
  ...
  struct
    ...
    let encoding =
      ...
      let with_tuple =
        obj1
          (opt "devices" (list Device.encoding))
      in
      conv to_tuple of_tuple with_tuple
  end
