...
struct
  ...
  struct
    ...
    let encoding =
      ...
      let with_tuple =
        obj2
          (req "identifier" Identifier.encoding)
          (req "password" string)
      in
      conv to_tuple of_tuple with_tuple

    let pp ppf t =
      Fmt.(pf ppf "{ identifier: %a; password: %s }"
        (Identifier.pp) t.identifier
        t.password)
    end
...
let encoding =
    ...
    let with_tuple =
      cond
        ...
        [ Some "m.login.dummy", case Dummy.encoding (function Dummy t -> Some t | _ -> None) (fun t -> Dummy t)
        ; Some "m.login.token", case Token.encoding (function Token t -> Some t | _ -> None) (fun t -> Token t)
        ; Some "m.login.password", case Password.encoding (function Password t -> Some t | _ -> None) (fun t -> Password t)
        ; None, case Empty.encoding (function Empty t -> Some t | _ -> None) (fun t -> Empty t) ]
    in
    conv to_tuple of_tuple with_tuple
