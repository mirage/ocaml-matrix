...
struct
  ...
  struct
    ...
    struct
      ...
      let encoding =
        let to_tuple t =
          t.user_id, t.device_id, t.algorithms, t.keys, t.signatures
        in
        let of_tuple v =
    ...
    struct
      type t =
        | Key of string
        | Object_key of Repr.value

      let encoding =
        union
          [ case string (function Key t -> Some t | _ -> None) (fun t -> Key t)
          ; case any (function Object_key t -> Some t | _ -> None) (fun t -> Object_key t) ]
    end
    ...
    let encoding =
      ...
      let with_tuple =
        obj2
          ...
          (opt "one_time_keys" (assoc Keys_format.encoding))
      in
      conv to_tuple of_tuple with_tuple
  end
  ...
  struct
    ...
    let encoding =
      ...
      let with_tuple =
        obj1
          (req "one_time_key_counts" (assoc int))
      in
      conv to_tuple of_tuple with_tuple
  end
...
struct
  ...
  struct
    ...
    let encoding =
      let to_tuple t =
        t.timeout, t.device_keys, t.token
      in
      let of_tuple v =
    ...
    struct
      ...
      let encoding =
        ...
        let of_tuple v =
          ...
          { user_id; device_id; algorithms; keys; signatures; unsigned }
        in
        let with_tuple =
          obj6
            (req "user_id" string)
            ...
            (req "keys" (assoc string))
            (req "signatures" (assoc (assoc string)))
            (opt "unsigned" Unsigned_device_info.encoding)
        in
        conv to_tuple of_tuple with_tuple
    end
    ...
    let encoding =
      ...
      let with_tuple =
        obj2
          ...
          (opt "device_keys" (assoc (assoc Device_keys.encoding)))
      in
      conv to_tuple of_tuple with_tuple
  end
  ...
  struct
    ...
    let encoding =
      ...
      let with_tuple =
        obj2
          (req "timeout" int)
          (req "one_time_keys" (assoc (assoc string)))
      in
      conv to_tuple of_tuple with_tuple
  end
...
struct
  ...
  struct
    ...
    let encoding =
      ...
      let with_tuple =
        obj2
          (opt "changed" (list string))
          (opt "left" (list string))
      in
      conv to_tuple of_tuple with_tuple
  end
