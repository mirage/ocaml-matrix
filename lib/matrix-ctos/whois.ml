...
struct
  ...
  struct
    ...
    struct
      ...
      let with_tuple =
        obj1
          (opt "connections" (list Connection_info.encoding))
      in
      conv to_tuple of_tuple with_tuple
    end
    ...
    let encoding =
      ...
      let with_tuple =
        obj1
          (opt "sessions" (list Session_info.encoding))
      in
      conv to_tuple of_tuple with_tuple
  end
  ...
  let encoding =
    ...
    let with_tuple =
      obj2
        (opt "user_id" string)
        (opt "devices" (assoc Device_info.encoding))
    in
    conv to_tuple of_tuple with_tuple
end
