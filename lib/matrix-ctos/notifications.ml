...
struct
  ...
  struct
    ...
    let encoding =
      let to_tuple t =
        t.actions, t.event, t.profile_tag, t.read, t.room_id, t.ts
      in
      let of_tuple v =
  ...
  let encoding =
    let to_tuple t =
      t.next_token, t.notifications
    in
    let of_tuple v =
      let next_token, notifications = v in
      { next_token; notifications }
    in
    let with_tuple =
      obj2
        (opt "next_token" Null.string)
        (req "notifications" (list Notification.encoding))
    in
    conv to_tuple of_tuple with_tuple
end
