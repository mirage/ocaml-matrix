...
struct
  ...
  struct
    ...
    struct
      ...
      let encoding =
        let to_tuple t =
          t.limit, t.not_senders, t.not_types, t.senders, t.types, t.lazy_load_members, t.include_redundant_members, t.not_rooms, t.rooms, t.contains_url
        in
        let of_tuple v =
          let limit, not_senders, not_types, senders, types, lazy_load_members, include_redundant_members, not_rooms, rooms, contains_url = v in
          { limit; not_senders; not_types; senders; types; lazy_load_members; include_redundant_members; not_rooms; rooms; contains_url }
        in
        let with_tuple =
          obj10
            (opt "limit" int)
    ...
    struct
      ...
      let encoding =
        let to_tuple t =
          t.before_limit, t.after_limit, t.include_profile
        in
        let of_tuple v =
        ...
        let with_tuple =
          obj3
            (opt "before_limit" int)
            (opt "after_limit" int)
            (opt "include_profile" bool)
        in
        conv to_tuple of_tuple with_tuple
    end
    ...
    struct
      ...
      let encoding =
        ...
        let with_tuple =
          obj1
            (opt "group_by" (list Group.encoding))
        in
        conv to_tuple of_tuple with_tuple
    end
    ...
    let encoding =
      let to_tuple t =
        t.search_term, t.keys, t.filter, t.order_by, t.event_context, t.include_state, t.groupings
      in
      let of_tuple v =
        let search_term, keys, filter, order_by, event_context, include_state, groupings = v in
        { search_term; keys; filter; order_by; event_context; include_state; groupings }
      in
      let with_tuple =
        obj7
          (req "search_term" string)
          (opt "keys" Key.encoding)
          ...
          (opt "groupings" Groupings.encoding)
      in
        conv to_tuple of_tuple with_tuple
  end
  ...
  let encoding =
    ...
    let with_tuple =
      obj1
        (req "search_categories"
          (obj1
            (opt "room_events" Criteria.encoding)))
    in
    conv to_tuple of_tuple with_tuple
end
...
struct
  ...
  struct
    ...
    struct
      ...
      let encoding =
        let to_tuple t =
          t.next_batch, t.order, t.results
        in
        let of_tuple v =
          let next_batch, order, results = v in
          { next_batch; order; results }
        in
        let with_tuple =
          obj3
    ...
    let encoding =
      let to_tuple t =
        t.count, t.highlights, t.results, t.state, t.groups, t.next_batch
      in
      let of_tuple v =
        let count, highlights, results, state, groups, next_batch = v in
        { count; highlights; results; state; groups; next_batch }
      in
      let with_tuple =
        obj6
          (opt "count" int)
  ...
  let encoding =
    ...
    let with_tuple =
      obj1
        (req "search_categories"
          (obj1
            (opt "room_events" Results.encoding)))
    in
    conv to_tuple of_tuple with_tuple
end
