...
module Event = struct
  type t = {event_content: Event_content.t} [@@deriving accessor]
  ...
  let encoding =
    let to_tuple t = get_type t.event_content, t.event_content in
    let of_tuple v =
...
module Pdu = struct
  type t = {event: event; prev_events: string list; depth: int}
  [@@deriving accessor]

  let encoding =
    let to_tuple t = t.event, (t.prev_events, t.depth) in
    let of_tuple v =
      let event, (prev_events, depth) = v in
      {event; prev_events; depth} in
    let with_tuple =
      merge_objs encoding
        (obj2 (req "prev_events" (list string)) (req "depth" int)) in
    conv to_tuple of_tuple with_tuple
