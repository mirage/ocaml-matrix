...
struct
    ...
    let encoding =
      ...
      let with_tuple =
        obj4
          ...
          (opt "predecessor" Previous_room.encoding)
      in
      conv to_tuple of_tuple with_tuple
  end
  ...
  struct
    ...
    let encoding =
      let to_tuple t =
        t.avatar_url, t.displayname, t.membership, t.is_direct, t.reason, t.third_party_invite
      in
      let of_tuple v =
        let avatar_url, displayname, membership, is_direct, reason, third_party_invite = v in
        {avatar_url; displayname; membership; is_direct; reason; third_party_invite}
      in
      let with_tuple =
        obj6
          ...
          (opt "is_direct" bool)
          (opt "reason" string)
          (opt "third_party_invite" Third_party_invite.encoding)
      in
      conv to_tuple of_tuple with_tuple
  end
    ...
    let encoding =
      ...
      let with_tuple =
        obj10
          ...
          (opt "notifications" Notifications.encoding)
      in
      conv to_tuple of_tuple with_tuple
  end
  ...
  struct
    ...
    let encoding =
      ...
      let with_tuple =
        ...
        in
        obj1
          (req "history_visibility" rule_encoding)
      in
      conv to_tuple of_tuple with_tuple
  end
        ...
        let encoding =
          ...
          let with_tuple =
            obj7
              ...
              (opt "thumbnail_info" Message.Thumbnail.encoding)
          in
          conv to_tuple of_tuple with_tuple
      end
      ...
      let encoding =
        ...
        let with_tuple =
          obj4
            ...
            (req "url" string)
            (opt "file" Message.Encrypted_file.encoding)
        in
        conv to_tuple of_tuple with_tuple
    end
  ...
  struct
    ...
    let encoding =
      let to_tuple t =
        t.avatar_url, t.displayname, t.last_active_ago, t.presence, t.currently_active, t.status_msg
      in
      let of_tuple v =
        let avatar_url, displayname, last_active_ago, presence, currently_active, status_msg = v in
        { avatar_url; displayname; last_active_ago; presence; currently_active; status_msg }
      in
      let with_tuple =
        obj6
  ...
  let encoding =
    union
      [ case Aliases.encoding            (function Aliases t -> Some t | _ -> None)            (fun t -> Aliases t)
      ; case Canonical_alias.encoding    (function Canonical_alias t -> Some t | _ -> None)    (fun t -> Canonical_alias t)
      ; case Create.encoding             (function Create t -> Some t | _ -> None)             (fun t -> Create t)
      ; case Join_rules.encoding         (function Join_rules t -> Some t | _ -> None)         (fun t -> Join_rules t)
      ; case Member.encoding             (function Member t -> Some t | _ -> None)             (fun t -> Member t)
      ; case Power_levels.encoding       (function Power_levels t -> Some t | _ -> None)       (fun t -> Power_levels t)
      ; case History_visibility.encoding (function History_visibility t -> Some t | _ -> None) (fun t -> History_visibility t)
      ; case Third_party_invite.encoding (function Third_party_invite t -> Some t | _ -> None) (fun t -> Third_party_invite t)
      ; case Guest_access.encoding       (function Guest_access t -> Some t | _ -> None)       (fun t -> Guest_access t)
      ; case Server_acl.encoding         (function Server_acl t -> Some t | _ -> None)         (fun t -> Server_acl t)
      ; case Tombstone.encoding          (function Tombstone t -> Some t | _ -> None)          (fun t -> Tombstone t)
      ; case Encryption.encoding         (function Encryption t -> Some t | _ -> None)         (fun t -> Encryption t)
      ; case Encrypted.encoding          (function Encrypted t -> Some t | _ -> None)          (fun t -> Encrypted t)
      ; case Message.encoding            (function Message t -> Some t | _ -> None)            (fun t -> Message t)
      ...
      ; case Presence.encoding           (function Presence t -> Some t | _ -> None)           (fun t -> Presence t)
      ; case Push_rules.encoding         (function Push_rules t -> Some t | _ -> None)         (fun t -> Push_rules t)
      ; case Typing.encoding             (function Typing t -> Some t | _ -> None)             (fun t -> Typing t)
      ; case Receipt.encoding            (function Receipt t -> Some t | _ -> None)            (fun t -> Receipt t)
      ; case Fully_read.encoding         (function Fully_read t -> Some t | _ -> None)         (fun t -> Fully_read t)
      ; case Tag.encoding                (function Tag t -> Some t | _ -> None)                (fun t -> Tag t)
      ; case Direct.encoding             (function Direct t -> Some t | _ -> None)             (fun t -> Direct t)
      ; case Room_key.encoding           (function Room_key t -> Some t | _ -> None)           (fun t -> Room_key t)
      ; case Room_key_request.encoding   (function Room_key_request t -> Some t | _ -> None)   (fun t -> Room_key_request t)
      ; case Forwarded_room_key.encoding (function Forwarded_room_key t -> Some t | _ -> None) (fun t -> Forwarded_room_key t)
      ; case Dummy.encoding              (function Dummy t -> Some t | _ -> None)              (fun t -> Dummy t)
      ]
end
...
struct
  ...
  let get_type : Event_content.t -> string = function
  ...
  let encoding =
    let to_tuple t =
      get_type t.event_content, t.event_content
    in
    let of_tuple v =
      let _, event_content = v in
      { event_content }
    in
    let with_tuple =
      (cond
        (obj1 (req "type" string))
        [ "m.room.aliases",            case (Content.content Event_content.Aliases.encoding)            (function Event_content.Aliases t -> Some t | _ -> None)            (fun t -> Aliases t)
        ; "m.room.canonical_alias",    case (Content.content Event_content.Canonical_alias.encoding)    (function Event_content.Canonical_alias t -> Some t | _ -> None)    (fun t -> Canonical_alias t)
        ; "m.room.create",             case (Content.content Event_content.Create.encoding)             (function Event_content.Create t -> Some t | _ -> None)             (fun t -> Create t)
        ; "m.room.join_rules",         case (Content.content Event_content.Join_rules.encoding)         (function Event_content.Join_rules t -> Some t | _ -> None)         (fun t -> Join_rules t)
        ; "m.room.member",             case (Content.content Event_content.Member.encoding)             (function Event_content.Member t -> Some t | _ -> None)             (fun t -> Member t)
        ; "m.room.power_levels",       case (Content.content Event_content.Power_levels.encoding)       (function Event_content.Power_levels t -> Some t | _ -> None)       (fun t -> Power_levels t)
        ; "m.room.history_visibility", case (Content.content Event_content.History_visibility.encoding) (function Event_content.History_visibility t -> Some t | _ -> None) (fun t -> History_visibility t)
        ; "m.room.third_party_invite", case (Content.content Event_content.Third_party_invite.encoding) (function Event_content.Third_party_invite t -> Some t | _ -> None) (fun t -> Third_party_invite t)
        ; "m.room.guest_access",       case (Content.content Event_content.Guest_access.encoding)       (function Event_content.Guest_access t -> Some t | _ -> None)       (fun t -> Guest_access t)
        ; "m.room.server_acl",         case (Content.content Event_content.Server_acl.encoding)         (function Event_content.Server_acl t -> Some t | _ -> None)         (fun t -> Server_acl t)
        ; "m.room.tombstone",          case (Content.content Event_content.Tombstone.encoding)          (function Event_content.Tombstone t -> Some t | _ -> None)          (fun t -> Tombstone t)
        ; "m.room.encryption",         case (Content.content Event_content.Encryption.encoding)         (function Event_content.Encryption t -> Some t | _ -> None)         (fun t -> Encryption t)
        ; "m.room.encrypted",          case (Content.content Event_content.Encrypted.encoding)          (function Event_content.Encrypted t -> Some t | _ -> None)          (fun t -> Encrypted t)
        ; "m.room.message",            case (Content.content Event_content.Message.encoding)            (function Event_content.Message t -> Some t | _ -> None)            (fun t -> Message t)
        ; "m.room.name",               case (Content.content Event_content.Name.encoding)               (function Event_content.Name t -> Some t | _ -> None)               (fun t -> Name t)
        ; "m.room.topic",              case (Content.content Event_content.Topic.encoding)              (function Event_content.Topic t -> Some t | _ -> None)              (fun t -> Topic t)
        ; "m.room.avatar",             case (Content.content Event_content.Avatar.encoding)             (function Event_content.Avatar t -> Some t | _ -> None)             (fun t -> Avatar t)
        ; "m.room.pinned_events",      case (Content.content Event_content.Pinned_events.encoding)      (function Event_content.Pinned t -> Some t | _ -> None)             (fun t -> Pinned t)
        ; "m.call.invite",             case (Content.content Event_content.Call.Invite.encoding)        (function Event_content.Invite t -> Some t | _ -> None)             (fun t -> Invite t)
        ; "m.call.candidates",         case (Content.content Event_content.Call.Candidates.encoding)    (function Event_content.Candidates t -> Some t | _ -> None)         (fun t -> Candidates t)
        ; "m.call.answer",             case (Content.content Event_content.Call.Answer.encoding)        (function Event_content.Answer t -> Some t | _ -> None)             (fun t -> Answer t)
        ; "m.call.hangup",             case (Content.content Event_content.Call.Hangup.encoding)        (function Event_content.Hangup t -> Some t | _ -> None)             (fun t -> Hangup t)
        ; "m.presence",                case (Content.content Event_content.Presence.encoding)           (function Event_content.Presence t -> Some t | _ -> None)           (fun t -> Presence t)
        ; "m.push_rules",              case (Content.content Event_content.Push_rules.encoding)         (function Event_content.Push_rules t -> Some t | _ -> None)         (fun t -> Push_rules t)
        ; "m.typing",                  case (Content.content Event_content.Typing.encoding)             (function Event_content.Typing t -> Some t | _ -> None)             (fun t -> Typing t)
        ; "m.receipt",                 case (Content.content Event_content.Receipt.encoding)            (function Event_content.Receipt t -> Some t | _ -> None)            (fun t -> Receipt t)
        ; "m.fully_read",              case (Content.content Event_content.Fully_read.encoding)         (function Event_content.Fully_read t -> Some t | _ -> None)         (fun t -> Fully_read t)
        ; "m.tag",                     case (Content.content Event_content.Tag.encoding)                (function Event_content.Tag t -> Some t | _ -> None)                (fun t -> Tag t)
        ; "m.direct",                  case (Content.content Event_content.Direct.encoding)             (function Event_content.Direct t -> Some t | _ -> None)             (fun t -> Direct t)
        ; "m.room_key",                case (Content.content Event_content.Room_key.encoding)           (function Event_content.Room_key t -> Some t | _ -> None)           (fun t -> Room_key t)
        ; "m.room_key_request",        case (Content.content Event_content.Room_key_request.encoding)   (function Event_content.Room_key_request t -> Some t | _ -> None)   (fun t -> Room_key_request t)
        ; "m.forwarded_room_key",      case (Content.content Event_content.Forwarded_room_key.encoding) (function Event_content.Forwarded_room_key t -> Some t | _ -> None) (fun t -> Forwarded_room_key t)
        ; "m.dummy",                   case (Content.content Event_content.Dummy.encoding)              (function Event_content.Dummy t -> Some t | _ -> None)              (fun t -> Dummy t)
        ])
    in
    conv to_tuple of_tuple with_tuple
end
...
struct
  ...
  struct
    ...
    let encoding =
      let to_tuple t =
        t.age, t.redacted_because, t.transaction_id
      in
      let of_tuple v =
        let age, redacted_because, transaction_id = v in
        { age; redacted_because; transaction_id }
      in
      let with_tuple =
        obj3
          (opt "age" int)
          (opt "redacted_because" Event.encoding)
          (opt "transaction_id" string)
      in
      conv to_tuple of_tuple with_tuple
  end
...
struct
  type t =
    { room_event: Room_event.t
    ; state_key: string
    } [@@deriving accessor]

  let encoding =
    let to_tuple t =
      t.room_event, t.state_key
    in
    let of_tuple v =
