...
module Path = struct
  ...
  let pp = Fmt.list print_path_item
...
and 'a bounds = {minimum: 'a option; maximum: 'a option}
  ...
  | Cond (t, cases) -> (
    ...
    fun v ->
      ...
      | `O fields -> (
        ...
        | Some (Case {encoding; inj; _}) -> r, inj (destruct encoding (`O rest))
        | None ->
          raise
            (Cannot_destruct
               ( []
...
let req n t = Req {name= n; encoding= t}
let opt n t = Opt {name= n; encoding= t}
let dft n t d = Dft {name= n; encoding= t; default= d}
let null = Null
let int = Int {minimum= None; maximum= None}
let ranged_int minimum maximum = Int {minimum; maximum}
let float = Float {minimum= None; maximum= None}
let ranged_float minimum maximum = Float {minimum; maximum}
let string = String
let conv ffrom fto t = Conv (ffrom, fto, t)
let bool = Bool
let array t = Array t
let obj1 f1 = Obj f1
...
let obj10 f1 f2 f3 f4 f5 f6 f7 f8 f9 f10 =
  conv
    ...
    (fun (a, (b, (c, (d, (e, (f, (g, (h, (i, j))))))))) ->
      a, b, c, d, e, f, g, h, i, j)
    (let rest =
       Objs (Obj f6, Objs (Obj f7, Objs (Obj f8, Objs (Obj f9, Obj f10)))) in
     Objs
       (Obj f1, Objs (Obj f2, Objs (Obj f3, Objs (Obj f4, Objs (Obj f5, rest))))))
...
let string_enum cases =
  ...
  let cases_str =
    String.concat " " (List.map (fun x -> "'" ^ fst x ^ "'") cases) in
  List.iter
    (fun (s, c) ->
...
let list t = Conv (Array.of_list, Array.to_list, Array t)
...
let merge_objs o1 o2 =
  ...
  let rec is_obj : type t. t encoding -> bool = function
    ...
    | Cond (k, cases) ->
      is_obj k
      && List.for_all (fun (_, Case {encoding= o; _}) -> is_obj o) cases
    | _ -> false in
...
let rec print_error ppf = function
  | Cannot_destruct ([], exn) -> print_error ppf exn
  | Cannot_destruct (path, Unexpected (unex, ex)) ->
    Fmt.pf ppf "At %a, unexpected %s instead of %s" Path.pp path unex ex
  | Cannot_destruct (path, No_case_matched errs) ->
    Fmt.pf ppf "@[<v 2>At %a, no case matched:@,%a@]" Path.pp path
      (Format.pp_print_list print_error)
      errs
  | Cannot_destruct (path, Bad_array_size (unex, ex)) ->
    Fmt.pf ppf "At %a, unexpected array of size %d instead of %d" Path.pp path
      unex ex
  | Cannot_destruct (path, Missing_field n) ->
    Fmt.pf ppf "At %a, missing object field %s" Path.pp path n
  | Cannot_destruct (path, Unexpected_field n) ->
    Fmt.pf ppf "At %a, unexpected object field %s" Path.pp path n
  | Cannot_destruct (path, Bad_schema exn) ->
    Fmt.pf ppf "@[<v 2>At %a, bad custom schema:@,%a@]" Path.pp path print_error
      exn
  | Unexpected (unex, ex) -> Fmt.pf ppf "Unexpected %s instead of %s" unex ex
  | No_case_matched errs ->
    Fmt.pf ppf "@[<v 2>No case matched:@,%a@]"
      (Format.pp_print_list print_error)
      errs
  | Bad_array_size (unex, ex) ->
    Fmt.pf ppf "Unexpected array of size %d instead of %d" unex ex
  | Missing_field n -> Fmt.pf ppf "Missing object field %s" n
  | Unexpected_field n -> Fmt.pf ppf "Unexpected object field %s" n
  | Bad_schema exn ->
    Fmt.pf ppf "@[<v 2>bad custom schema:@,%a@]" print_error exn
  | Cannot_destruct (path, exn) ->
    Fmt.pf ppf "@[<v 2>At %a:@,%a@]" Path.pp path print_error exn
  | exn -> Fmt.pf ppf "%a" Fmt.exn exn
