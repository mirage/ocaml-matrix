...
let set_state_event room_id event_type state_key event_id =
  ...
  Store.set store
    Key.(v "rooms" / room_id / "state" / event_type / state_key)
    event_id
...
let get_event_since key since =
  ...
  | Ok (_, last_modified) ->
    if Int64.of_int since < last_modified then Store.get store key
    else Lwt.return_error (`Not_found key)
...
let get_room_timeline room_id since =
  let rec f acc id =
    ...
    >>= function
    | Error (`Not_found _) -> Lwt.return_ok acc
    | Error err -> Lwt.return_error err
    | Ok next_id -> (
      match next_id with
      | "" -> Lwt.return_ok (id :: acc)
      | next_id -> f (id :: acc) next_id) in
  f [] "head" >>= function
  | Error err -> Lwt.return_error err
  | Ok messages ->
    Lwt_list.filter_map_p
      (fun message_id ->
        Event_store.get event_store Key.(v message_id) >>= function
        | Error _ -> Lwt.return_none
        | Ok event -> Lwt.return_some event)
      messages
    >>= fun events ->
    List.map (fun event -> destruct Events.Room_event.encoding event) events
    |> Lwt.return_ok
...
let get_room_ephemeral room_id _since =
  ...
  | Ok users ->
    Lwt_list.filter_map_p
      (fun (username, _) ->
        Store.get store
          Key.(v "rooms" / room_id / "ephemeral" / "typing" / username)
        >>= function
        | Error _ -> Lwt.return_none
        | Ok until ->
          let _until = float_of_string until in
          Lwt.return_some username)
      users
    >>= fun users ->
    Lwt.return_ok
      [
...
let get_room_state room_id since =
  Store.list store Key.(v "rooms" / room_id / "state") >>= function
  | Error err -> Lwt.return_error err
  | Ok event_types ->
    Lwt_list.filter_map_p
      (fun (event_type, _) ->
          ...
          Lwt_list.filter_map_p
            (fun (state_key, _) ->
              ...
              | Ok event_id -> (
                ...
                | Ok event -> Lwt.return_some event))
            state_keys
          >>= fun events -> Lwt.return_some events)
      event_types
    >>= fun events ->
    List.flatten events
    |> List.map (destruct Events.State_event.encoding)
    |> Lwt.return_ok
...
let get_rooms_membership user_id since =
  ...
  | Ok rooms ->
    ...
    let f (j, i, l) = function
      | room_id, Events.Event_content.Membership.Join, changed ->
        (room_id, changed) :: j, i, l
      | room_id, Events.Event_content.Membership.Invite, changed ->
        j, (room_id, changed) :: i, l
      | room_id, Events.Event_content.Membership.Ban, changed
      | room_id, Events.Event_content.Membership.Leave, changed ->
        j, i, (room_id, changed) :: l
      | _room_id, Events.Event_content.Membership.Knock, _ -> j, i, l
      (* maybe do something for knocked rooms *) in
    let rooms = List.fold_left f ([], [], []) rooms in
    Lwt.return_ok rooms

let get_rooms user_id since =
  let update = ref false in
  get_rooms_membership user_id since >>= function
  | Error err -> Lwt.return_error err
  | Ok (j, i, l) ->
    Lwt_list.filter_map_p
      (fun (room_id, changed) ->
        let since = if changed then 0 else since in
        get_room_state room_id since >>= function
        | Error _ -> Lwt.return_none
        | Ok state_events -> (
          get_room_timeline room_id since >>= function
          | Error _ -> Lwt.return_none
          | Ok message_events -> (
            get_room_ephemeral room_id since >>= function
            | Error _ -> Lwt.return_none
            | Ok ephemeral_events ->
              let timeline =
                Rooms.Timeline.make ?events:(Some message_events) () in
              let room =
                Rooms.Joined_room.make ?state:(Some state_events)
                  ?timeline:(Some timeline) ?ephemeral:(Some ephemeral_events)
                  () in
              update :=
                !update
