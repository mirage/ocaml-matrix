...
let parse uri meth routes =
  ...
  let rec find : type k. k t -> k -> string list -> (bool * endpoint) option = function
      ...
      (fun acc p ->
        match p with
          ...
          | x::t ->
            ...
            | None -> Option.map (fun node -> find node (acc, x) t) variable |> Option.join)
  in
  find routes () paths

let option uri routes =
  let paths = String.split_on_char '/' uri in
  let rec find : type k. k t -> k -> string list -> Code.meth list option = function
    | Node (meths, paths, variable) ->
      (fun acc p ->
        match p with
