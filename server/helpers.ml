...
let error errcode error =
  ...
  construct encoding error |> Ezjsonm.value_to_string
...
let ( >?= ) f g =
  Lwt.bind f (function
    ...
    | Error _ ->
      Lwt.return
        (`Internal_server_error, error "M_UNKNOWN" "Internal storage failure"))

let placeholder =
  let f _ _ _ _ =
    Lwt.return (`Not_implemented, "Not implemented", Some "text/html") in
  false, f

let deprecated =
  let f _ _ _ _ =
    Lwt.return (`Moved_permanently, "Not implemented", Some "text/html") in
  false, f

let username_to_user_id username = "@" ^ username ^ ":" ^ Const.homeserver
...
let get_logged_user = function
  ...
  | Some token -> (
    Store.exists store Key.(v "tokens" / token) >>= function
    | Error _ -> Lwt.return_error ()
    | Ok None -> Lwt.return_ok None
    | Ok (Some _) -> (
      Store.get store Key.(v "tokens" / token) >>= function
