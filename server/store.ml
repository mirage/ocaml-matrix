...
module Store : sig
  ...
  val list :
    t -> key -> ((string * [ `Dictionary | `Value ]) list, error) result Lwt.t
  ...
  let to_Fs_key k = Mirage_kv.Key.segments k |> Fs_store.Key.v
  let to_kv_value = function `Contents -> `Value | `Node -> `Dictionary
  ...
  let error = function Ok v -> Ok v | Error e -> Error (e :> error)
  ...
  let set t k v =
    ...
    Fs_store.set ~info t k v >|= write_error

  let exists t k =
    let k' = to_Fs_key k in
    let f () = Fs_store.kind t k' >|= Option.map to_kv_value >>= Lwt.return_ok in
    catch_error f k

  let get t k =
    let k' = to_Fs_key k in
    let f () = Fs_store.get t k' >>= Lwt.return_ok in
    catch_error f k
  ...
  let list t k =
    ...
    let f () =
      Fs_store.list t k'
      >|= List.map (fun (s, v) ->
              s, Fs_store.Tree.destruct v |> tree_to_kv_value)
      >>= Lwt.return_ok in
    catch_error f k

  (* This is a copy and paste from irmin code so I can adapt the behaviour of the
     last_modified function. Should be replaced asap *)

  module Heap = Binary_heap.Make (struct
    type t = Fs_store.commit * int

    let compare c1 c2 =
      -Int64.compare
         (Irmin.Info.date (Fs_store.Commit.info (fst c1)))
  ...
  let last_modified t k =
    let k' = to_Fs_key k in
    let f () =
      Fs_store.last_modified t k' >>= function
  ...
  let remove t k =
    let k = to_Fs_key k in
    Fs_store.remove ~info t k >|= write_error

  let store () =
    let config = Irmin_git.config "/tmp/ocaml-matrix-string" in
    Fs_store.Repo.v config >>= Fs_store.master

  let digest _ _ = assert false
  let batch _ = assert false
end

let store = Lwt_main.run (Store.store ())
...
module Event_store : sig
  ...
  let to_Fs_key k = Mirage_kv.Key.segments k |> Fs_event_store.Key.v
  let to_kv_value = function `Contents -> `Value | `Node -> `Dictionary
  ...
  let pp_error ppf = function
    | #Mirage_kv.error as e -> Mirage_kv.pp_error ppf e
    | #Fs_event_store.write_error as e ->
      Irmin.Type.pp Fs_event_store.write_error_t ppf e
  ...
  let set t k v =
    let k = to_Fs_key k in
    Fs_event_store.set ~info t k v >|= write_error

  let exists t k =
    let k' = to_Fs_key k in
    let f () =
      Fs_event_store.kind t k' >|= Option.map to_kv_value >>= Lwt.return_ok
    in
    catch_error f k

  let get t k =
    let k' = to_Fs_key k in
    let f () = Fs_event_store.get t k' >>= Lwt.return_ok in
    catch_error f k
  ...
  let list t k =
    ...
    let f () =
      ...
      >|= List.map (fun (s, v) ->
              s, Fs_event_store.Tree.destruct v |> tree_to_kv_value)
      >>= Lwt.return_ok in
    catch_error f k
  ...
  let remove t k =
    let k = to_Fs_key k in
    Fs_event_store.remove ~info t k >|= write_error

  let store () =
    let config = Irmin_git.config "/tmp/ocaml-matrix-event" in
    Fs_event_store.Repo.v config >>= Fs_event_store.master

  let digest _ _ = assert false
  let batch _ = assert false
end

let event_store = Lwt_main.run (Event_store.store ())
