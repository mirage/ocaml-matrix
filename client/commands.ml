...
module Server = struct
  ...
  let gen_signature key str =
    Rresult.R.error_msg_to_invalid_arg @@ Base64.encode ~pad:false @@ Cstruct.to_string @@ Mirage_crypto_ec.Ed25519.sign ~key (Cstruct.of_string str)
  ...
  let public_rooms key =
    ...
    Fmt.(
      pf stdout "%a\n%!"
        (styled `Cyan (styled `Bold string))
        "Get public rooms:")
    ; let json = Federation_request.make ~meth:"GET" ~uri:"/_matrix/federation/v1/publicRooms" ~origin:"ocaml-matrix" ~destination:"my.domain.name" () in
      let obj_key = read_key "/home/gwenaelle/.ssh/ed25519key.pem" in
      let response = Json_encoding.construct (Federation_request.encoding Response.encoding) json
      ...
      Lwt_main.run
        (get
           ~make_headers:
             (make_federation_headers "ocaml-matrix" key
                signed)
           "_matrix/federation/v1/publicRooms" (Query.args args)
    ...
    let join_room key room_id =
      ...
      Fmt.(
        pf stdout "%a\n%!"
          (styled `Cyan (styled `Bold string))
          "Join a room:")
      ; let content = Matrix_common.Events.Pdu.make ~event:(`Room_event Matrix_common.Events.(Room_event.make ~event:(Event.make ~event_content:(Event_content.(Member (Member.make ~membership:Join ()))) ())
          ~sender:"@flibidi:ocaml-matrix" ~origin:"ocaml-matrix" ~origin_server_ts:((Unix.time () |> Float.to_int) * 1000) ~room_id ())) ~prev_events:[] ~depth:25 () in
        let json = Federation_request.make ~meth:"PUT" ~uri ~origin:"ocaml-matrix" ~destination:"my.domain.name" ~content () in
        let obj_key = read_key "/home/gwenaelle/.ssh/ed25519key.pem" in
        let response = Json_encoding.construct (Federation_request.encoding Request.encoding) json
          |> Json_encoding.canonize
          |> Ezjsonm.value_to_string in
        let signed = gen_signature obj_key response in
        Lwt_main.run
          (put
             ~make_headers:
               (make_federation_headers "ocaml-matrix" key
                  signed)
             uri (Query.args args)
             content Request.encoding Response.encoding true)
end
