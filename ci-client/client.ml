...
let logout job host port auth_token =
  ...
  Http.post host port "_matrix/client/r0/logout" None (Request.make ()) Request.encoding
...
let resolve_alias job host port room_alias =
  ...
  Http.get host port (Fmt.str "/_matrix/client/r0/directory/room/%s" room_alias) None Response.encoding None
...
let run job _room ctx msg =
  login job ctx.host ctx.port (make_login ctx.device ctx.user ctx.pwd) >>= fun login_response ->
  let auth_token = Login.Post.Response.get_access_token login_response in
  resolve_alias job ctx.host ctx.port "#ocaml-matrix:my.domain.name" >>= fun resolved_alias ->
  let room_id = Option.get (Room.Resolve_alias.Response.get_room_id resolved_alias) in
  ...
  >>= fun () -> logout job ctx.host ctx.port auth_token
  >>= fun _ -> Lwt.return_ok ()
